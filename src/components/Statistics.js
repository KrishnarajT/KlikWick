...
const Statistics = () => {
	...
	const variance_score = {
		...
		light:
			spread_data.reduce(
				(acc, curr) =>
					acc + Math.pow(curr.light - average_score.light, 2),
				0
			) / spread_data.length,
		sound:
			spread_data.reduce(
				(acc, curr) =>
					acc + Math.pow(curr.sound - average_score.sound, 2),
				0
			) / spread_data.length,
	};
	...
	const skewness_score = {
		...
		light:
			spread_data.reduce(
				(acc, curr) =>
					acc + Math.pow(curr.light - average_score.light, 3),
				0
			) /
		...
		sound:
			spread_data.reduce(
				(acc, curr) =>
					acc + Math.pow(curr.sound - average_score.sound, 3),
				0
			) /
	...
	const kurtosis_score = {
		...
		light:
			spread_data.reduce(
				(acc, curr) =>
					acc + Math.pow(curr.light - average_score.light, 4),
				0
			) /
		...
		sound:
			spread_data.reduce(
				(acc, curr) =>
					acc + Math.pow(curr.sound - average_score.sound, 4),
				0
			) /
	...
	const z_score_score = spread_data.map((user) => {
		return {
			id: user.id,
			light:
				(user.light - average_score.light) / standard_deviation.light,
			sound:
				(user.sound - average_score.sound) / standard_deviation.sound,
		};
	});
	...
	const median_absolute_deviation_score = {
		...
		light: spread_data
			.reduce(
				(acc, curr) =>
					acc.concat(Math.abs(curr.light - median_score.light)),
				[]
			)
		...
		sound: spread_data
			.reduce(
				(acc, curr) =>
					acc.concat(Math.abs(curr.sound - median_score.sound)),
				[]
			)
			.sort((a, b) => a - b)[Math.floor(spread_data.length / 2)],
	};
